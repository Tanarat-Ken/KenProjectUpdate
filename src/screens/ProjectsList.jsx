        ...
        {data && (
          <>
            ...
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 13, overflow: 'hidden' }}>
              ...
              {/* Rows */}
              {filtered.length === 0 ? (
                <EmptyState title="ยังไม่มีงานในหมวดนี้" sub="Create a new project to get started" />
              ) : (
                filtered.map((p, i) => (
                  <div
                    key={p.id}
            ...
            {/* Legend */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, fontFamily: "'Sarabun'", fontSize: 11.5, color: C.grayLight, flexWrap: 'wrap' }}>
